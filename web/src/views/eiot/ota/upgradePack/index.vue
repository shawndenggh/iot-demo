  ...
  >
    <template #urlForm="{ row, type }">
      <file-upload
        ...
        :limit="1"
        :file-type="[]"
        uploadUrl="/eiot/ota/package/upload"
        uploadType="url"
        @upload-success="(list) => uploadSuccess(row, list)"
      />
      <a v-else :href="row.url" target="_blank">{{ row.url }}</a>
    </template>
    <template #productKeyForm="{ row }">
      <div style="width: 250px">
        <select-product v-model:pk="row.productKey" />
      </div>
    </template>
...
import { getUpgradePack, addUpgradePack, delUpgradePack } from '@/api/eiot/ota/upgradePack.api'
