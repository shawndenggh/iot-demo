<template>
  <div style="width: 100%">
    <el-form ref="formRef" class="model-form" :rules="rules" label-width="120px" :model="propertyRef">
      ...
      <el-form-item style="margin-bottom: 18px;" label="寄存器类型" prop="regType">
        <el-select v-model="propertyRef.regType" placeholder="请选择寄存器类型" style="width: 100%" @change="regTypeChange">
          <el-option v-for="item in regTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item style="margin-bottom: 18px;" label="寄存器地址" prop="regAddr">
        <el-input-number v-model="propertyRef.regAddr" auto-complete="off" placeholder="十进制地址" style="width: 100%" min="0" step="1" step-strictly />
      </el-form-item>
      <el-form-item style="margin-bottom: 18px;" label="寄存器数量" prop="regNum">
      ...
      <el-form-item style="margin-bottom: 18px;" label="数据顺序" prop="sort">
        <el-select v-model="propertyRef.sort" placeholder="请选择处理公式" style="width: 100%">
          <el-option v-for="item in sortOptions" :key="item.value" :label="item.value" :value="item.value" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item style="margin-bottom: 10px;" v-if="isProperty&&!isSelectType('position')" label="读写" prop="accessMode">
        <el-radio-group v-model="propertyRef.accessMode" disabled>
...
const rules = reactive({
  ...
  'regAddr': [
    { required: true, message: '请输入寄存器地址', trigger: 'change' },
  ],
  'regNum': [
    { required: true, message: '请输入寄存器数量', trigger: 'change' },
  ],
...
const regTypeChange=(value: string | number)=>{
  if(value=='01'||value=='03'){
    propertyRef.value.accessMode='rw'
  }
  if(value=='02'||value=='04'){
    propertyRef.value.accessMode='r'
  }
}
