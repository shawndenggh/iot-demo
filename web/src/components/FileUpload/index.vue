...
const handleBeforeUpload = (file: any) => {
  ...
  if (props.fileType.length) {
    ...
    if (!isTypeOk) {
      message.alertError(`文件格式不正确, 请上传${props.fileType.join('/')}格式文件!`)
  ...
  if (props.fileSize) {
    ...
    if (!isLt) {
      message.alertError(`上传文件大小不能超过 ${props.fileSize} MB!`)
      return false
    }
  }
...
// 文件个数超出
const handleExceed = () => {
  message.alertError(`上传文件数量不能超过 ${props.limit} 个!`)
}

// 上传失败
const handleUploadError = () => {
  message.alertError('上传文件失败')
}
