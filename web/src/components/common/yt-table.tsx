...
export default defineComponent({
  ...
  setup(props, { emit, slots, expose }) {
    ...
    const renderMenus = (scope: { row: any }) => {
      return (
        <div class="menu-cell flex justify-center">
          {props.viewBtn && (
            <div class="menu-btn">
      ...
      const renDiv = (scope: IScope) => {
        ...
        if (!column.slot) {
          switch (column.type) {
            ...
            case 'switch':
              const { activeValue, inactiveValue } = column.componentProps || {}
              return <ElSwitch modelValue={row[column.key]} activeValue={activeValue === 0 ? 0 : true} inactiveValue={inactiveValue === 0 ? 0 : false} />
            case 'image':
              return (
                <ElImage
    ...
    return () => (
      <div style="overflow: hidden;">
        ...
        {!props.pageHide && (
          <Pagination total={props.total} v-model:page={pageObj.pageNo} v-model:limit={pageObj.pageSize} onPagination={changePage} />
        )}
      </div>
    )
  },
