...
or with uv
uv run --with simple-ddl-parser convertor.py postgres > ../postgresql/enjoy-iot.sql
uv run --with simple-ddl-parser convertor.py sqlserver > ../sqlserver/enjoy-iot.sql
uv run --with simple-ddl-parser convertor.py kingbase > ../kingbase/enjoy-iot.sql
uv run --with simple-ddl-parser convertor.py opengauss > ../opengauss/enjoy-iot.sql
uv run --with simple-ddl-parser convertor.py oracle > ../oracle/enjoy-iot.sql
uv run --with simple-ddl-parser convertor.py dm8 > ../dm/enjoy-iot-dm8.sql
"""
...
"""
        ...
        for table_sql in self.table_script_list:
            ...
            table_ddl["comment"] = bytes(table_ddl["comment"], "utf-8").decode(
                "unicode_escape"
            )[1:-1] if table_ddl.get("comment") else None
...
def main():
    ...
    sql_file = pathlib.Path("../mysql/system.sql").resolve().as_posix()
    convertor = None
